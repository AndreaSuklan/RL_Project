...
from dqn import MyDQN, SimpleNet, SimpleDQN  
from stable_baselines3 import PPO as PPO_Model, MyDQN as DQN_Model
...
def train(algorithm, timesteps, debug=False):
    """
    ...
    if algorithm == 'ppo':
        ...
        agent = MyDQN(env=env, policy=SimpleDQN(), debug)
    else:
        print(f"Error: Unknown algorithm '{algorithm}'")
        env.close()
        return
    print(f"--- Created {algorithm} agent. Starting learning ---") 
    # Train the agent
...
if __name__ == '__main__':
    ...
    parser.add_argument('debug', action='store_true', help='Debugging flag for extra printing')

    args = parser.parse_args()

    if args.action == "train":
        train(args.algorithm, args.timesteps, args.debug)
