...
def plot_learning_curve(df, title, filename):
    ...
    # Create a unique identifier for each run for proper rolling average
    df['run_id'] = df['algorithm'] + '_' + df['model'] + '_' + df['seed'].astype(str)
    df['smoothed_reward'] = df.groupby('run_id')['reward'].transform(
...
def plot_metric(df, metric_col, title, filename, y_log_scale=False):
    ...
    metric_df['run_id'] = metric_df['algorithm'] + '_' + metric_df['model'] + '_' + metric_df['seed'].astype(str)
    metric_df[metric_col + '_smoothed'] = metric_df.groupby('run_id')[metric_col].transform(
        lambda x: x.rolling(ROLLING_WINDOW, min_periods=1).mean()
    )

    # Use 'episode' for the x-axis
    sns.lineplot(data=metric_df, x='episode', y=metric_col + '_smoothed', hue='algorithm', style="model", errorbar='sd', legend=True)
...
def main():
    ...
    # --- Generate All Plots ---
    plot_learning_curve(full_df, "Learning Curve Comparison", "reward_curve.png")
